...
class CustomerPriceModule:
    ...
    def html(self) -> str:
        ...
        return """<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>APC Core · Customer Price</title><style>:root{--ink:#24272b;--muted:#68717c;--line:#e5e1db;--cream:#faf7f2;--paper:#fffdfa;--accent:#1d6b57;--warn:#8a6417;--list-alt:#f1ede4;--list-hover:#dcefe5}*{box-sizing:border-box}body{margin:0;background:var(--cream);color:var(--ink);font:14px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.shell{max-width:1240px;margin:auto;padding:26px}.toolbar{display:grid;grid-template-columns:1fr 1fr auto auto auto;gap:9px;padding:12px 0}input,textarea,button{font:inherit;border:1px solid var(--line);border-radius:10px;padding:10px;background:#fffdfa}button{cursor:pointer;font-weight:700}button:disabled{opacity:.55;cursor:not-allowed}.primary{background:var(--accent);color:#fff}.workspace{border:1px solid var(--line);border-radius:18px;background:var(--paper);overflow:hidden}.hint,.provenance{color:var(--muted)}.summary{padding:14px 18px;border-bottom:1px solid var(--line);display:flex;justify-content:space-between}.rows{width:100%;border-collapse:collapse}.rows th,.rows td{padding:12px 14px;border-bottom:1px solid var(--line);text-align:left}.rows th{font-size:11px;text-transform:uppercase;color:var(--muted)}.rows tbody tr:nth-child(even){background:var(--list-alt)}.rows tbody tr:hover,.rows tbody tr:focus-within{background:var(--list-hover);outline:2px solid #b9dbcf;outline-offset:-2px}.rows input{width:110px;padding:7px}.modal-backdrop{position:fixed;inset:0;z-index:5;display:grid;place-items:center;padding:18px;background:#24272b66}.modal{width:min(720px,100%);max-height:calc(100vh - 36px);overflow:auto;padding:20px;border:2px solid #d7c98d;border-radius:14px;background:#fffaf0;box-shadow:4px 4px 0 #d7c98d}.modal textarea{display:block;width:100%;min-height:180px;margin:10px 0}.modal-actions{display:flex;gap:9px;justify-content:flex-end}.preview{margin-top:12px;display:grid;gap:8px}.preview-card{padding:9px 11px;border-left:4px solid var(--warn);background:#fff}.preview-card.good{border-color:var(--accent)}[hidden]{display:none!important}@media(max-width:820px){.shell{padding:14px}.toolbar{grid-template-columns:1fr 1fr}.toolbar label{grid-column:span 2}.summary{display:block}}</style><body><main class="shell"><a href="../">← APC Core</a><h1>Customer Price</h1><p class="hint">Keyboard-first customer prices. Choose a Customer Code, then search its imported item-price rows.</p><section class="toolbar"><label>Customer Code <input id="customer-code" list="customer-options" role="combobox" aria-autocomplete="list" aria-controls="customer-options" autocomplete="off" placeholder="Type a customer code"></label><datalist id="customer-options"></datalist><label>Search item-price rows <input id="search" type="search"></label><button id="reload" type="button">Reload</button><button id="edit" type="button" disabled>Edit prices</button><button id="bulk" type="button" disabled>Bulk edit</button></section><section class="workspace"><div class="summary"><strong id="status" aria-live="polite">Choose a Customer Code</strong><span class="provenance">Accepted snapshot provenance retained per row</span></div><table class="rows"><thead><tr><th>Item ID</th><th>Item</th><th>Price</th><th>Provenance</th><th id="row-action" hidden>Action</th></tr></thead><tbody id="rows"></tbody></table></section></main><div id="bulk-dialog" class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="bulk-title" hidden><section class="modal"><h2 id="bulk-title">Bulk edit prices</h2><p class="hint">Paste <b>Item ID</b> then <b>Price</b>, tab-separated. A conventional header row is optional and is excluded automatically.</p><textarea id="tsv" aria-label="Paste Item ID and Price rows" placeholder="IT-001&#9;14.00"></textarea><div class="modal-actions"><button id="close-bulk" type="button">Cancel</button><button id="preview" type="button">Preview</button><button id="apply" class="primary" type="button" disabled>Apply</button></div><div id="preview-result" class="preview" aria-live="polite"></div></section></div><script>(()=>{const $=id=>document.getElementById(id),customer=$("customer-code"),options=$("customer-options"),search=$("search"),rows=$("rows"),status=$("status"),edit=$("edit"),bulk=$("bulk"),dialog=$("bulk-dialog"),tsv=$("tsv"),result=$("preview-result"),apply=$("apply"),action=$("row-action");let preview=null,editMode=false,lastFocus,lastPage=null,customerCodes=[];const selected=()=>customer.value.trim(),active=()=>window.apcCoreActiveStaff||"";const request=(path,method="GET",body)=>fetch(path,{method,headers:body?{"Content-Type":"application/json"}:{},body:body?JSON.stringify(body):undefined}).then(async r=>{const p=await r.json();if(!r.ok)throw Error(p.error||"Request failed");return p});const text=(tag,value)=>{const n=document.createElement(tag);n.textContent=value;return n};const clear=n=>n.replaceChildren();function sync(){const ready=!!selected();edit.disabled=!ready;bulk.disabled=!ready;action.hidden=!editMode;edit.textContent=editMode?"Done editing":"Edit prices"}function render(p){lastPage=p;clear(rows);status.textContent=p.total+" item-price rows";for(const r of p.rows){const tr=document.createElement("tr"),price=text("td",r.price),cell=document.createElement("td");tr.append(text("td",r.item_id),text("td",r.item_description),price,text("td",r.provenance));if(editMode){const input=document.createElement("input"),save=document.createElement("button");input.value=r.price;input.inputMode="decimal";input.setAttribute("aria-label","Price for "+r.item_id);save.textContent="Save";save.onclick=async()=>{if(!active()){status.textContent="Choose a user before saving.";return}try{await request("api/customers/"+encodeURIComponent(selected())+"/items/"+encodeURIComponent(r.item_id),"POST",{price:input.value,actor:active()});load()}catch(e){status.textContent=e.message}};price.replaceChildren(input);cell.append(save)}tr.append(cell);rows.append(tr)}sync()}function load(){const code=selected();lastPage=null;preview=null;apply.disabled=true;if(!code){clear(rows);status.textContent="Choose a Customer Code";sync();return}request("api/customers/"+encodeURIComponent(code)+"?q="+encodeURIComponent(search.value)).then(p=>{if(selected()===code)render(p)}).catch(e=>{if(selected()===code){status.textContent=e.message;sync()}})}function showPreview(p){preview=p;clear(result);for(const [kind,values] of Object.entries({valid:p.valid,invalid:p.invalid,unknown:p.unknown,duplicate:p.duplicate,changes:p.changes})){const card=document.createElement("div");card.className="preview-card"+(kind==="valid"||kind==="changes"?" good":"");card.append(text("strong",kind+": "+values.length));for(const r of values)card.append(text("div",kind==="changes"?r.item_id+": "+r.before+" → "+r.after:(r.item_id||"line "+r.line)+": "+(r.reason||r.price)));result.append(card)}apply.disabled=!(p.valid.length||p.changes.length)||p.invalid.length>0||p.unknown.length>0||p.duplicate.length>0}function openBulk(){if(!selected()){status.textContent="Choose a Customer Code";customer.focus();return}lastFocus=bulk;dialog.hidden=false;tsv.focus()}function closeBulk(){dialog.hidden=true;lastFocus?.focus()}request("api/customers").then(p=>{customerCodes=p.customer_codes||[];customerCodes.forEach(code=>{const o=document.createElement("option");o.value=code;options.append(o)})});tsv.oninput=()=>{preview=null;apply.disabled=true;clear(result)};function commitCustomer(){const typed=selected().toLowerCase(),match=customerCodes.find(code=>code.toLowerCase()===typed)||customerCodes.find(code=>code.toLowerCase().startsWith(typed));if(match)customer.value=match;load()}customer.addEventListener("change",commitCustomer);customer.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key==="Tab"){e.preventDefault();commitCustomer();if(e.key==="Tab")setTimeout(()=>search.focus(),0)}if(e.key==="Escape"){customer.value="";load()}});search.addEventListener("input",()=>{clearTimeout(search.timer);search.timer=setTimeout(load,160)});$("reload").onclick=load;edit.onclick=()=>{if(selected()){editMode=!editMode;if(lastPage)render(lastPage);else load()}};bulk.onclick=openBulk;$("close-bulk").onclick=closeBulk;$("preview").onclick=()=>request("api/customers/"+encodeURIComponent(selected())+"/paste/preview","POST",{tsv:tsv.value}).then(showPreview).catch(e=>{preview=null;apply.disabled=true;status.textContent=e.message});apply.onclick=()=>{if(!active()||!preview){status.textContent="Choose a user and preview before Apply.";return}request("api/customers/"+encodeURIComponent(selected())+"/paste/apply","POST",{preview_id:preview.preview_id,actor:active()}).then(p=>{status.textContent=p.applied+" rows applied";closeBulk();load()}).catch(e=>status.textContent=e.message)};document.addEventListener("keydown",e=>{if(e.key==="Escape"&&!dialog.hidden){e.preventDefault();closeBulk()}if((e.ctrlKey||e.metaKey)&&e.key==="Enter"&&!dialog.hidden){e.preventDefault();$("preview").click()}});sync();window.addEventListener("apc-core-identity",()=>{})})()</script></body></html>"""
