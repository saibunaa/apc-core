...
def _staff_identity_shell(html: str) -> str:
    ...
    shell = """<section id="identity-confirm" aria-live="polite" style="position:fixed;right:16px;top:16px;z-index:10"><div id="identity-picker" class="identity-card" style="width:min(360px,calc(100vw - 32px));padding:20px;border:1px solid #e5e1db;border-radius:18px;background:#fffdfa;box-shadow:0 16px 42px rgba(36,39,43,.14)"><p style="margin:0 0 14px"><strong style="display:block;font-size:16px">Who is using APC Program?</strong><span style="display:block;margin-top:5px;color:#68717c;line-height:1.4">Activity attribution only — not security, authentication, or authorization.</span></p><label style="display:grid;gap:6px;font-weight:600">Choose user <select id="identity-user" style="min-height:38px;border:1px solid #e5e1db;border-radius:10px;padding:0 10px;background:#fff"></select></label><button id="confirm-user" type="button" style="margin-top:14px;min-height:38px;border:0;border-radius:10px;padding:0 14px;background:#1d6b57;color:#fff;font-weight:700">Continue</button></div><button id="identity-change-user" type="button" hidden>Change user</button></section><div id="identity-content" hidden>"""
