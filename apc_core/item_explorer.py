...
_BANGKOK_RATE_PATH = "/customers/api/rates/bangkok-bank"
_BANGKOK_RATE_DATE = re.compile(r"\d{4}-\d{2}-\d{2}")
_BANGKOK_RATE_SLOT = re.compile(r"[1-9]\d*")
_BANGKOK_RATE_MAX_SLOT = 1440


def _bangkok_rate_query(query: str) -> tuple[str, int]:
    """Accept exactly one calendar date and one bounded update slot."""
    try:
        pairs = parse_qsl(query, keep_blank_values=True, strict_parsing=True, max_num_fields=2)
    except ValueError as error:
        raise ValueError("invalid Bangkok Bank rate query") from error
    if len(pairs) != 2 or {key for key, _ in pairs} != {"date", "update_slot"}:
        raise ValueError("invalid Bangkok Bank rate query")
    values = dict(pairs)
    selected_date, update_slot = values["date"], values["update_slot"]
    if not _BANGKOK_RATE_DATE.fullmatch(selected_date) or not _BANGKOK_RATE_SLOT.fullmatch(update_slot):
        raise ValueError("invalid Bangkok Bank rate query")
    try:
        date.fromisoformat(selected_date)
        slot = int(update_slot)
    except ValueError as error:
        raise ValueError("invalid Bangkok Bank rate query") from error
    if slot > _BANGKOK_RATE_MAX_SLOT:
        raise ValueError("invalid Bangkok Bank rate query")
    return selected_date, slot


def _bangkok_rate_dto(selected_date: str, update_slot: int, snapshot: object) -> dict:
    """Project an injected snapshot onto the intentionally small customer DTO."""
    selected_at = snapshot.selected_at
    usd, sgd = snapshot.usd, snapshot.sgd
    values = (
        selected_at.isoformat(),
        usd.currency_label, usd.column_label, usd.thb_per_unit,
        sgd.currency_label, sgd.column_label, sgd.thb_per_unit,
        snapshot.usd_to_sgd,
    )
    if not all(type(value) is str for value in values):
        raise ValueError("invalid Bangkok Bank rate snapshot")
    return {
        "date": selected_date,
        "update_slot": update_slot,
        "selected_at": values[0],
        "rates": {
            "usd": {"currency_label": values[1], "column_label": values[2], "thb_per_unit": values[3]},
            "sgd": {"currency_label": values[4], "column_label": values[5], "thb_per_unit": values[6]},
            "usd_to_sgd": values[7],
        },
    }
...
def make_handler(
    explorer: ItemExplorer,
    manifest: dict,
    customer_explorer=None,
    *,
    customer_lan_ingress: bool = False,
    bangkok_rate_service=None,
):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            customer_read_path = (
                ...
                or parsed.path == _BANGKOK_RATE_PATH
                or parsed.path == "/customers/api/customers"
                or parsed.path.startswith("/customers/api/customers/")
            )
            if (customer_explorer is not None or parsed.path == _BANGKOK_RATE_PATH) and customer_read_path and not _customer_client_allowed(self.client_address[0], customer_lan_ingress):
            ...
            if customer_explorer is not None and parsed.path == "/customers/":
                ...
                self.send_response(HTTPStatus.OK); self.send_header("Content-Type", "text/html; charset=utf-8"); self.send_header("Cache-Control", "no-store"); self.send_header("Content-Length", str(len(body))); self.end_headers(); self.wfile.write(body); return
            if parsed.path == _BANGKOK_RATE_PATH:
                try:
                    selected_date, update_slot = _bangkok_rate_query(parsed.query)
                except ValueError:
                    self._send_json(HTTPStatus.BAD_REQUEST, {"error": "invalid Bangkok Bank rate query"})
                    return
                if bangkok_rate_service is None:
                    self._send_json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": "Bangkok Bank rate service is not configured"})
                    return
                try:
                    snapshot = bangkok_rate_service.selected_rate(selected_date, update_slot)
                    self._send_json(HTTPStatus.OK, _bangkok_rate_dto(selected_date, update_slot, snapshot))
                except Exception:
                    self._send_json(HTTPStatus.BAD_GATEWAY, {"error": "Bangkok Bank rate service failed"})
                return
