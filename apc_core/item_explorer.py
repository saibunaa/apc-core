...
def _customer_explorer_html() -> str:
    ...
    html = html.replace("clean(results);d.customers.forEach(c=>results.append(button('',c.customer_id+' · '+c.name,()=>open(c.customer_id),'customer')));", "clean(results);customerCodes=d.customers.map(c=>c.customer_id);const codeOptions=$('#customer-code-options');codeOptions.replaceChildren(...customerCodes.map(code=>{const option=document.createElement('option');option.value=code;return option}));d.customers.forEach(c=>results.append(button('',c.customer_id+' · '+c.name,()=>open(c.customer_id),'customer')));")
    html = html.replace("$('#search').onclick=load;q.onkeydown=e=>{if(e.key==='Enter')load()};", "$('#search').onclick=load;function commitCustomerCode(){const typed=q.value.trim().toLowerCase(),match=customerCodes.find(code=>code.toLowerCase()===typed)||customerCodes.find(code=>code.toLowerCase().startsWith(typed));if(match){q.value=match;open(match)}else load()}q.onkeydown=e=>{if(e.key==='Enter'||e.key==='Tab'){e.preventDefault();commitCustomerCode();if(e.key==='Tab')setTimeout(()=>profile.focus(),0)}};")
