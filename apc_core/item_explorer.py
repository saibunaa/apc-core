...
def _staff_identity_shell(html: str) -> str:
    ...
    shell = """<script>try{window.apcCoreKnownUser=!!localStorage.getItem("apc-core-identity");if(window.apcCoreKnownUser)document.documentElement.classList.add("apc-core-known-user")}catch(_){}</script><style>html.apc-core-known-user #identity-picker{display:none}html.apc-core-known-user #identity-content{display:block!important}.identity-card{width:min(390px,calc(100vw - 32px));padding:20px;border:1px solid #e5e1db;border-radius:18px;background:#fffdfa;box-shadow:0 16px 42px rgba(36,39,43,.14)}.identity-tiles{display:grid;gap:7px;margin-top:14px}.identity-tile{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;width:100%;min-height:52px;padding:10px 12px;border:1px solid #e5e1db;border-radius:11px;background:#fffdfa;color:#24272b;text-align:left;cursor:pointer}.identity-tile:hover,.identity-tile:focus-visible{border-color:#1d6b57;background:#edf7f1;outline:3px solid #b9dbcf;outline-offset:2px}.identity-tile[aria-checked="true"]{border-left:4px solid #1d6b57}.identity-role{color:#68717c;font-size:12px}.identity-action{min-height:38px;border:1px solid #1d6b57;border-radius:999px;padding:0 14px;background:#fffdfa;color:#1d6b57;font-weight:700;cursor:pointer}</style><section id="identity-confirm" aria-live="polite" style="position:fixed;right:16px;top:16px;z-index:10"><div id="identity-picker" class="identity-card"><p style="margin:0"><strong style="display:block;font-size:16px">Who is using APC Program?</strong><span style="display:block;margin-top:5px;color:#68717c;line-height:1.4">Activity attribution only — not security, authentication, or authorization.</span></p><div id="identity-tiles" class="identity-tiles" role="radiogroup" aria-label="Active staff"></div></div><button id="identity-change-user" type="button" class="identity-action" hidden>Change</button><script>if(window.apcCoreKnownUser)document.getElementById("identity-change-user").hidden=false</script></section><div id="identity-content" hidden>"""
