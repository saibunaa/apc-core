...
class TestCustomerExplorerContract(unittest.TestCase):
    ...
    def test_backfill_preserves_legacy_price_type_lexically_in_dedicated_raw_field(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = self.make_snapshot(
                root,
                [("C-RAW", "Raw factor", "", "", "", "", " 1.20 ", "", "", "", "")],
            )
            explorer = CustomerExplorer(source, data_dir=root / "state")

            explorer.backfill_from_snapshot()
            customer = explorer.profile("C-RAW")["customer"]

        self.assertEqual(" 1.20 ", customer["price_type"])
        self.assertEqual(" 1.20 ", customer["legacy_price_type_raw"])

    def test_core_created_customer_does_not_claim_legacy_price_type_provenance(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            explorer = CustomerExplorer(self.make_snapshot(root), data_dir=root / "state")
            explorer.create({"customer_id": "C-CORE", "name": "Core customer", "price_type": "1.20"}, "YIM")
            customer = explorer.profile("C-CORE")["customer"]

        self.assertEqual("1.20", customer["price_type"])
        self.assertEqual("", customer["legacy_price_type_raw"])

    def test_refresh_updates_raw_source_value_while_preserving_core_price_type_edit(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            initial = self.make_snapshot(
                root,
                [("C-REFRESH", "Refresh", "", "", "", "", "1.20", "", "", "", "")],
            )
            explorer = CustomerExplorer(initial, data_dir=root / "state")
            explorer.backfill_from_snapshot()
            explorer.edit("C-REFRESH", {"price_type": "Core-entered"}, "YIM")
            later = root / "later.sqlite"
            later.write_bytes(initial.read_bytes())
            connection = sqlite3.connect(later)
            connection.execute('UPDATE "MainDB__CUST" SET "Price Type"=? WHERE "Cust ID"=?', ("1.2", "C-REFRESH"))
            connection.commit()
            connection.close()
            explorer.close()
            refreshed = CustomerExplorer(later, data_dir=root / "state")
            refreshed.refresh_from_snapshot()
            customer = refreshed.profile("C-REFRESH")["customer"]

        self.assertEqual("Core-entered", customer["price_type"])
        self.assertEqual("1.2", customer["legacy_price_type_raw"])

    def test_old_schema_migration_keeps_core_created_legacy_raw_empty(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
            state.mkdir()
            connection = sqlite3.connect(state / "apc_core.sqlite")
            customer_columns = ", ".join(f"{field} TEXT NOT NULL DEFAULT ''" for field in (
                "name", "address_1", "address_2", "address_3", "tel", "fax", "email", "price_type", "box_type", "invoice_header", "invoice_type", "invoice_year",
            ))
            connection.execute(
                "CREATE TABLE core_customers (customer_id TEXT PRIMARY KEY, source_customer_id TEXT, source_artifact_path TEXT, "
                "source_artifact_sha256 TEXT, imported_at TEXT, core_created INTEGER NOT NULL, archived INTEGER NOT NULL DEFAULT 0, "
                + customer_columns + ")"
            )
            connection.execute("INSERT INTO core_customers(customer_id,core_created,price_type) VALUES (?,?,?)", ("C-OLD", 1, "1.20"))
            connection.commit()
            connection.close()
            explorer = CustomerExplorer(self.make_snapshot(root), data_dir=state)
            customer = explorer.profile("C-OLD")["customer"]
